...
Data flow: `cli` parses argv (`parseCliArgs`, a pure function in its own file so tests can import it without executing the entry) and guards on terminal capability. If stdout is not a TTY it prints a notice and exits 0 (CI-friendly). `--fallback` and `--render-mode kitty` are contradictory (one forces the cell renderer, the other forces the full player) and combining them is a usage error. If `detectFallbackReasons` reports missing placeholder support or a tmux/screen session, it warns and asks whether to continue in fallback mode. Declining exits 0. `--fallback` skips the prompt (it also forces fallback on a supported terminal), and any other `--render-mode` implies fallback the same way, since a cell mode has no meaning in the Ink player. `--render-mode kitty` forces the full player past detection. Either path then opens the `FrameSource` (`ffmpegSource` when a file argument is given, the built-in `proceduralSource` otherwise). In fallback mode `runFallbackPlayer` drives a probe-free cell-renderer Screen (mode from `detectCellRenderMode`, cell-background on Terminal.app, half-block elsewhere, unless forced by `--render-mode`) with its own clock and raw-stdin keys, and never renders Ink. Otherwise the cli computes the panel `region` with `computePanelRegion`, and awaits kitty-motion's `createScreen` (with `renderMode: 'kitty'` when forced, otherwise following detection) BEFORE calling Ink's `render()`. That ordering is load-bearing because the capability probes read responses from stdin, and Ink's `useInput` takes stdin over once rendering starts. Then it renders `<Video screen source info autoPlay loop controls keyboard title help />` with `exitOnCtrlC: false` so Video's own input handler can dispose the Screen and close the source before Ink tears down.
...
- `src/fallbackPlayer/` - cell-renderer fallback playback (mode auto-detected via `detectCellRenderMode`, cell-background on Terminal.app, half-block elsewhere, or forced via `--render-mode`) for unsupported terminals: probe-free `createFallbackScreen` (full-screen, autoResize) plus `runFallbackPlayer`, a React-free port of the playback clock with raw-stdin keys (space, arrows, q/Ctrl-C) and no Ink UI
